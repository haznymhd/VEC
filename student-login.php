...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = "SELECT password FROM users WHERE id = ? AND user_type = 'student'"; // Update table name and add user_type condition
        ...
        if ($result) {
            if (mysqli_num_rows($result) === 1) {
                ...
                if (password_verify($password, $hashedPassword)) {
                    ...
                    header("Location: dash-board/student-dash/student-dashboard.php"); // Redirect to student dashboard
                    exit();
                } else {
...
<html>
<head>
  <title>Student Login - Virtual Examination Center</title>
